...
用法:
  Terminal 1: 启动 Franka 实机控制环境
  Terminal 2: python run_no_rcm_real.py --strategy online_priority --controller-mode pareto_iter
"""
import argparse
import os
import time
...
class Config:
    ...
    allow_sensorless_approach = False
...
def read_contact_force(force_sensor, venv, x, delta, delta_dot):
    """读取接触力；传感器新鲜时优先，否则回退到虚拟 Kelvin-Voigt 环境。"""
    ...
    return F_z, np.zeros(6), "virtual" if venv is not None else "none", False
...
def run_trial(robot, kin_tool, kin_flange,
    ...
    while not rospy.is_shutdown() and z_contact is None:
        ...
        F_raw, wrench, force_source, sensor_available = read_contact_force(
            force_sensor, venv, tp[0], contact_delta_from_surface(cfg, tp[2]), -tv[2]
        )
        F_z = float(force_filter.update(F_raw, dt))

        contact_force_valid = sensor_available or cfg.allow_sensorless_approach
        if (not surface_touched and contact_force_valid
                and abs(F_z) > cfg.contact_force_threshold):
        ...
        sensor_ready_for_descent = sensor_available or cfg.allow_sensorless_approach
        if not sensor_ready_for_descent:
            if sensor_unavailable_since is None:
                sensor_unavailable_since = now_wall
            # 传感器不可用时冻结 z 参考，避免控制器继续把工具向下压。
            z_ref_approach = max(z_ref_approach, float(tp[2]))
            xdot_ref = np.zeros(3)
            if force_sensor is not None:
                hold_msg = (
                    f"  Force sensor unavailable; holding approach z at "
                    f"{z_ref_approach:.4f}m instead of descending "
                    f"(source={force_source}, age={force_sensor.age():.4f}s)"
                )
            else:
                hold_msg = (
                    "  Force sensor unavailable; holding approach z instead of descending"
                )
            rospy.logwarn_throttle(1.0, hold_msg)
            if now_wall - sensor_unavailable_since > cfg.sensor_unavailable_abort_time:
                rospy.logerr(
        ...
        tau, _, _, integ_euler, _ = compute_torque_no_rcm(
            ...
            integ_euler=integ_euler, dt=dt_approach,
        )
        ...
        if rospy.Time.now().to_sec() - t0 > approach_timeout:
            rospy.logwarn(
                ...
                f"source={force_source}, sensor_available={sensor_available}, "
...
def main():
    ...
    ap.add_argument('--force-timeout', type=float, default=0.02,
                    help='direct sensor freshness window')
    ...
    ap.add_argument('--allow-sensorless-approach', action='store_true',
                    help='允许传感器不可用时继续按虚拟环境接近；默认禁止下探并中止')
    args = ap.parse_args()
    ...
    cfg = Config()
    cfg.allow_sensorless_approach = args.allow_sensorless_approach
    ...
    if not args.no_force_sensor:
        ...
        rospy.loginfo(
            ...
            f"allow_sensorless_approach={cfg.allow_sensorless_approach}"
        )
    else:
        rospy.logwarn(
            "Force sensor disabled; approach descent will be held unless "
            "--allow-sensorless-approach is set"
        )

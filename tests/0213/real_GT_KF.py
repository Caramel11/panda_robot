...
if __name__ == "__main__":
    ...
    target_points = {
        "a": np.array([0.35, -0.04, 0.007]),  # 目标a
        "b": np.array([0.35, -0.04, 0.03]),  # 目标a
        "c": np.array([0.35, 0.01, 0.03]),  # 目标c
        "d": np.array([0.35, 0.01, 0.02]),  # 目标d
    }
    ...
    while not rospy.is_shutdown():
        ...
        if current_target_idx < len(target_order) - 1 and (
            ...
            if current_target_idx == 1:
                ...
                num_point = int(
                    np.linalg.norm(robot_state["tool_position"] - current_target)
                    / 0.00005
                )

                way_plan2 = np.linspace(
                    robot_state["tool_position"], current_target, num_point
                )
        ...
        # print("error:", error)
        print(
            f"timepass: {timepass:.5f}, Goal: {target_order[current_target_idx]}, Error RCM: {error[0]:.5f}, Error Track: {error[1]:.5f}, 当前目标：{target_order[current_target_idx]}点，误差：{target_error:.6f},alpha: {alpha}, Fh: {callback.force_falcon},u: {u}"
        )

...
if __name__ == "__main__":
    ...
    target_points = {
        "a": np.array([0.35, 0.01, 0.007]),  # 目标a
        "b": np.array([0.35, 0.01, 0.03]),  # 目标a
        "c": np.array([0.35, 0.06, 0.03]),  # 目标c
        "d": np.array([0.35, 0.06, 0.02]),  # 目标d
    }
    ...
    while not rospy.is_shutdown():
        ...
        if current_target_idx < len(target_order) - 1 and (
            ...
            if current_target_idx == 1:
                ...
                #     current_target
                # )
                current_target=robot_state["tool_position"]+np.array([0,0,0.03])
        ...
        position_direct = position_falcon * scale
        vel_direct = vel_falcon * scale
        ...
        # print("beta:", beta)
        alpha = callback.HumanArbitrary()
        # print("alpha:", alpha)
        beta = 1
        arbitrary = np.array([alpha, beta])

        zref_h_array = zref_h_array_direct

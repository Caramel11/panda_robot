...
root = "/home/liu/moveit_ws_20240902/logs/0127/0127_MPC_Sigmoid"  # log存储根目录
init_error_threshold = 0.005  # 初始化误差阈值
tracking_error_threshold = 0.015  # 跟踪误差阈值
target_error_threshold = 0.015  # 目标点误差阈值
title_str = "MPC_Sigmoid"  # 日志文件标题字符串
u_threshold = 20  # 控制输入阈值
...
if __name__ == "__main__":
    ...
    target_points = {
        "a": np.array([0.25, -0.04, 0.005]),  # 目标a
        "b": np.array([0.25, -0.04, 0.03]),  # 目标b
        "c": np.array([0.3, 0.01, 0.03]),  # 目标c
        "d": np.array([0.3, 0.01, 0.02]),  # 目标d
    }
    object_position = np.array([0.25, -0.05, 0.05])
    object_radius = 0.03
    ...
    while not rospy.is_shutdown():
        ...
        if current_target_idx < len(target_order) - 1 and (
            ...
            if current_target_idx == 1:
                ...
                #     current_target
                # )
                current_target = tool_position_ref + np.array([0, 0, 0.03])
        ...
        u_r = controller.compute_control(
            state, Fh, i, if_rcm=True, ws_PSI=ws_PSI, ws_deform=ws_deform, PSI=0.5
        )

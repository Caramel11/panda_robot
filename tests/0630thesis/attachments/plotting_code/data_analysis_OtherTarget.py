"""
data_analysis_OtherTarget.py  (IEEE-style refactor for Figure 8)
================================================================
Generates `significance_annotated_OtherTarget.png` for the local-correction vs.
target-switching physical comparison.

Layout: 2x3 grid sized for `\\begin{figure*}` (double-column).
"""
...
method_disp = {
    "GT_KF":                  "Local Correction",
    "GT_KF_WithOtherTarget":  "Target Switching",
}

target_pairs = [("GT_KF", "GT_KF_WithOtherTarget")]
...
def make_figure(df, results_df):
    ...
    box_colors = {
        "GT_KF":                  IEEE_COLORS[2],  # green
        "GT_KF_WithOtherTarget":  IEEE_COLORS[3],  # purple
    }
    ...
    for i, (k, label) in enumerate(metrics.items()):
        ...
        # the original seaborn behaviour.
        data_per_method = [

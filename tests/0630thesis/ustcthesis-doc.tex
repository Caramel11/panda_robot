...
2017 年，随着学校发布了新版的《撰写手册》，本模板更新到 v3.0。
2025 年，研究生移除了《撰写手册》，使用了新的模板规范，模板更新直 v4.0。
...
本模板要求使用 TeX Live、MacTeX 或 MiKTeX 不低于 2021 年的发行版，
推荐升级到最新的版本。
...
\end{latex}
模板提供的选项见表~\ref{tab:title-page-options}，
\begin{table}
  \centering\small
  \caption{录入封面信息的选项}
  \label{tab:title-page-options}
  \begin{tabular}{ll}
    \toprule
    选项                      & 说明             \\
    \midrule
...
\begin{itemize}
  ...
  \item 提交日期 \opt{date} 应使用 ISO 格式（yyyy-mm-dd），默认为当前日期。
\end{itemize}

\DescribeMacro{\declarationoforiginality}
声明页由 \cs{declarationoforiginality} 命令生成。模板会根据 \opt{secret-level} 和
\opt{secret-year} 的内容自动填写。

用户使用 \cs{declarationoforiginality} 命令时也可以添加可选参数 \opt{file}，
插入签名、扫描后的声明页。
\begin{latex}
  \declarationoforiginality[file=figures/scan-statement.pdf]
\end{latex}

...
机器人运动学描述关节变量与末端位姿之间的几何关系，动力学描述关节力矩、惯性、重力、外部作用力与运动之间的关系。相关基础在机器人学手册和操作空间控制文献中已有系统总结\cite{siciliano2008springer}。对于受约束机器人操作任务，运动学为几何约束建模提供基础，动力学为控制器设计和稳定性分析提供对象。
...
由于后续控制目标主要在任务空间中给定，还需要把关节空间动力学转写到任务空间。
...
在几何约束之外，接触与人机交互还要求机器人具有可调柔顺性，阻抗控制提供了这一基础。
...
在线性二次型最优控制之后，再引入能够描述多目标协同的微分博弈语言。
...
为了描述多个目标之间的折中关系，还需要引入帕累托最优和折扣代价。
...
最后，为后续处理接触残差和模型失配，给出扰动与模型误差的基本分析工具。

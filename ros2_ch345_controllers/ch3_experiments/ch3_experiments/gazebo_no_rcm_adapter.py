"""第三章 no-RCM Gazebo 在线实验适配器。

本文件把历史稳定的 `no_rcm` 控制器封装成可由第三章实验脚本调用的函数：
启动 Gazebo、等待 effort 控制器 active、运行指定策略、停止进程并触发分析。
上层脚本不需要关心 ROS2 shell 环境和进程组管理细节。
"""

import os
import signal
import subprocess
import time
from pathlib import Path


WORKSPACE = Path("/home/liu/franka_ros2_ws")


def ros_bash(command):
    """生成带 ROS2 和工作区环境 source 的 bash 命令。"""

    return (
        "set -e\n"
        "source /opt/ros/humble/setup.bash\n"
        f"source {WORKSPACE}/install/setup.bash\n"
        f"{command}\n"
    )


def start_gazebo_no_rcm(world=None, gui=False, log_file=None):
    """启动第三章 no-RCM Gazebo 仿真环境。"""

    world = world or (
        WORKSPACE
        / "install/franka_gazebo_bringup/share/franka_gazebo_bringup/worlds/empty_no_gravity.sdf"
    )
    gz_args = f"-r {world}" if gui else f"-r -s {world}"
    cmd = ros_bash(
        "ros2 launch franka_gazebo_bringup gazebo_franka_arm_example_controller.launch.py "
        "robot_type:=fr3 "
        "load_gripper:=false "
        "rviz:=false "
        f"gz_args:=\"{gz_args}\" "
        "controller:=no_rcm_effort_controller"
    )
    stdout = None
    if log_file:
        Path(log_file).parent.mkdir(parents=True, exist_ok=True)
        stdout = Path(log_file).open("w", encoding="utf-8")
    return subprocess.Popen(
        ["bash", "-lc", cmd],
        cwd=str(WORKSPACE),
        stdout=stdout or subprocess.DEVNULL,
        stderr=subprocess.STDOUT if stdout else subprocess.DEVNULL,
        text=True,
        preexec_fn=os.setsid,
    )


def stop_process_group(proc):
    """停止由本适配器启动的 Gazebo/launch 进程组。"""

    if proc and proc.poll() is None:
        os.killpg(os.getpgid(proc.pid), signal.SIGINT)
        try:
            proc.wait(timeout=12)
        except subprocess.TimeoutExpired:
            os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
            proc.wait(timeout=8)


def wait_for_controller(timeout=45.0, process=None):
    """轮询 ros2_control，直到 no_rcm effort 控制器 active。"""

    deadline = time.time() + timeout
    while time.time() < deadline:
        if process is not None and process.poll() is not None:
            return False
        cmd = ros_bash("ros2 control list_controllers || true")
        out = subprocess.run(
            ["bash", "-lc", cmd],
            cwd=str(WORKSPACE),
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
            timeout=10,
            check=False,
        ).stdout
        if "no_rcm_effort_controller" in out and "active" in out:
            return True
        time.sleep(1.0)
    return False


def run_no_rcm_strategy(strategy, output_dir, trials=1, controller_mode="pareto_iter", benchmark="default", timeout=180):
    """调用 no_rcm 主程序运行一个控制策略。"""

    cmd = ros_bash(
        "ros2 run no_rcm run_no_rcm "
        f"--strategy {strategy} "
        f"--controller-mode {controller_mode} "
        f"--trials {int(trials)} "
        f"--benchmark {benchmark} "
        f"--output-dir {output_dir} "
        "--no-auto-plot "
        "--plot-no-show "
        "--ros-args "
        "-p cmd_topic:=/no_rcm_effort_controller/commands "
        "-p state_topic:=/joint_states "
        "-p rsp_node:=/robot_state_publisher "
        "-p gravity_compensation_scale:=0.0"
    )
    return subprocess.run(
        ["bash", "-lc", cmd],
        cwd=str(WORKSPACE),
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        text=True,
        check=False,
        timeout=timeout,
    )


def analyze_latest_result(output_root):
    """对最新 no_rcm 结果目录运行第三章分析脚本。"""

    root = Path(output_root)
    candidates = sorted(p for p in root.glob("no_rcm_*") if p.is_dir())
    if not candidates:
        raise FileNotFoundError(f"no no_rcm_* result directory under {root}")
    latest = candidates[-1]
    cmd = ros_bash(
        "ros2 run no_rcm analyze_ch3_result "
        f"--input {latest} "
        f"--output-dir {latest / 'ch3_analysis'}"
    )
    result = subprocess.run(
        ["bash", "-lc", cmd],
        cwd=str(WORKSPACE),
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        text=True,
        check=False,
    )
    return latest, result

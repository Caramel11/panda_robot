...
class PandaKinematics(object):
    ...
    def create_chain(self, ee_frame_name):
        ...
        self._ik_p_kdl = PyKDL.ChainIkSolverPos_NR(
            self._arm_chain, self._fk_p_kdl, self._ik_v_kdl
        )
    ...
    def print_robot_description(self):
        ...
        print(
            "URDF links: %d" % len(self._franka.links),
    ...
    def forward_position_kinematics(self, joint_values=None):
        ...
        self._fk_p_kdl.JntToCart(
            self.joints_to_kdl("positions", joint_values), end_frame
        )
        pos = end_frame.p
    ...
    def get_link_positions(self, joint_values=None):
        """
        获取所有连杆的笛卡尔位姿（位置 + 四元数姿态）
        :param joint_values: 关节角度字典或数组，若为None则使用当前关节角度
        :return: 字典 {link_name: [x, y, z, qx, qy, qz, qw]}
        """
        # 转换为KDL关节数组
        joints_kdl = self.joints_to_kdl("positions", joint_values)

        # 初始化存储结果
        link_poses = {}

        # 递归计算每个连杆的位姿
        fk_solver = PyKDL.ChainFkSolverPos_recursive(self._arm_chain)
        frame = PyKDL.Frame()
        ...
        for i in range(self._arm_chain.getNrOfSegments()):
            # 计算到第i个连杆末端的位姿
            fk_solver.JntToCart(
                joints_kdl, frame, i + 1
            )  # i+1表示计算到第i个Segment末端

            # 提取位置和姿态（四元数）
            pos = frame.p
            rot = frame.M
            rot_quat = rot.GetQuaternion()

            # 获取连杆名称
            link_name = self._arm_chain.getSegment(i).getName()
